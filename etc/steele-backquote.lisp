;;; Common Lisp the Language, 2nd Edition, Appendix C.
...
;; myself the behavior of nested backquotes. It implements the formal
;; rules for backquote processing and optionally applies a code
;; simplifier to the result. One must be very careful in choosing the
;; simplification rules; the rules given here work, but some Common
;; Lisp implementations have run into trouble at one time or another
;; by using a simplification rule that does not work in all
;; cases. Code transformations that are plausible when single forms
...
;; (setq s '(4 6))
;;
;; Without simplification, the notation $$(%%q) (which stands for
;; ``(,,q)) is read as the expression
;;
...
;; (LIST (R S))
;;
;; and the value of this value is (24). Thus the two ways of reading
;; $$(%%q) do not produce the same expression-this we expected-but the
;; values of the two ways are different as well. Only the values of
;; the values are the same. In general, Common Lisp guarantees the
;; result of an expression with backquotes nested to depth k only
;; after k successive evaluations have been performed; the results
;; after fewer than k evaluations are implementation-dependent.
;;
;; (Note that in the expression `(foo ,(process `(bar ,x))) the
;; backquotes are not doubly nested. The inner backquoted expression
;; occurs within the textual scope of a comma belonging to the outer
;; backquote. The correct way to determine the backquote nesting level
;; of any subexpression is to start a count at zero and proceed up the
;; S-expression tree, adding one for each backquote and subtracting
;; one for each comma. This is similar to the rule for determining
;; nesting level with respect to parentheses by scanning a character
;; string linearly, adding or subtracting one as parentheses
;; are passed.)
;;
;; It is convenient to extend the ``=='' notation to handle multiple
;; evaluation: x == == y means that the expressions x and y may have
;; different results but they have the same results when twice
;; evaluated. Similarly, x == ==== y means that the values of the
;; values of the values of x and y are the same, and so on.
;;
...
;; simplified form. (Actually, the unsimplified forms do contain one
;; simplification without which they would be unreadable: the nil that
;; terminates each list has been systematically suppressed, so that
;; one sees (append x y) rather than (append x y 'nil).)
;;
;; The following driver function is useful for tracing the behavior of
;; nested backquote syntax through multiple evaluations. The argument
;; ls is a list of strings; each string will be processed by the
;; reader (read-from-string). The argument n is the number of
;; evaluations desired.
;;
...
;;   (format t "~\&"))
;;
;; This driver routine makes it easy to explore a large number of
...
;; (setq s '((union x y)))
;;
;; Here is what happened when I executed (try fools2 2) with a non-nil
;; value for the variable *bq-simplify* (to see simplified forms). I
;; have interpolated some remarks.
;;
;; ``(foo ,,p) = (LIST 'LIST ''FOO P)
;;  => (LIST 'FOO (UNION X Y))
;;  => (FOO (A B C))
;;
;; So ,,p means ``the value of p is a form; use the value of the value
;; of p.''
;;
...
;;  => (FOO UNION X Y)
;;
;; So ,@',r means ``the value of r must be a list; splice in the value
;; of r that is available at the time of first evaluation, that is,
;; when the outer backquote is evaluated.'' (To splice the value of r
...
;; is, when the outer backquote is evaluated.'' (To splice the element
;; of the value of s that is available at the time of second
